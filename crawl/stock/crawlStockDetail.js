...
const crawlDetailHnx = asyncHandler(async (name, symbol, reference, ceil, floor, currentPrice, high, low, change, changePercent, turnOver) => {
    // cron.schedule('*/20 * * * * *', async () =>{
    ...
    try {
        const browser = await puppeteer.launch({ headless: true })
        const page = await browser.newPage()
        let status = await page.goto(`https://finance.vietstock.vn/${symbol}/tai-chinh.htm`, { timeout: 0 })
        // await page.click('[data-tooltip = "CTCP Xi măng Bỉm Sơn"]')
        // await page.click(`[data-value = ${symbol}]`)
        // const selector = await page.$(`#sym-328`)
        // await page.waitForSelector(`span[data-value=${symbol}]`)
        ...
        let hnxDetailData = await page.evaluate(async (name, symbol, reference, ceil, floor, currentPrice, high, low, change, changePercent, turnOver) => {
            ...
            try {
                ...
                dataJson.openPrice = document.getElementById('openprice')?.innerText
                dataJson.turnOver = turnOver
                dataJson.marketcap = document.querySelector('.stock-price-info :nth-child(2) :nth-child(5) b')?.innerText
                dataJson.overBought = document.querySelector('.stock-price-info :nth-child(3) :nth-child(1) b')?.innerText
                dataJson.overSold = document.querySelector('.stock-price-info :nth-child(3) :nth-child(2) b')?.innerText
                dataJson.high52Week = document.querySelector('.stock-price-info :nth-child(3) :nth-child(3) b')?.innerText
                dataJson.low52Week = document.querySelector('.stock-price-info :nth-child(3) :nth-child(4) b')?.innerText
                dataJson.turnOver52WeekAverage = document.querySelector('.stock-price-info :nth-child(3) :nth-child(5) b')?.innerText
                dataJson.foreignBuy = document.querySelector('.stock-price-info :nth-child(4) :nth-child(1) b')?.innerText
                dataJson.ownedRatio = document.querySelector('.stock-price-info :nth-child(4) :nth-child(2) b')?.innerText
                dataJson.dividendCast = document.querySelector('.stock-price-info :nth-child(4) :nth-child(3) b')?.innerText
                dataJson.dividendYield = document.querySelector('.stock-price-info :nth-child(4) :nth-child(4) b')?.innerText
                dataJson.beta = document.querySelector('.stock-price-info :nth-child(4) :nth-child(5) b')?.innerText
                dataJson.eps = document.querySelector('.stock-price-info :nth-child(5) :nth-child(1) b')?.innerText
                dataJson.pe = document.querySelector('.stock-price-info :nth-child(5) :nth-child(2) b')?.innerText
                dataJson.fpe = document.querySelector('.stock-price-info :nth-child(5) :nth-child(3) b')?.innerText
                dataJson.bvps = document.querySelector('.stock-price-info :nth-child(5) :nth-child(4) b')?.innerText
                dataJson.pb = document.querySelector('.stock-price-info :nth-child(5) :nth-child(5) b')?.innerText
            } catch (err) {
                console.log(err)
            }
            return dataJson
        }, name, symbol, reference, ceil, floor, currentPrice, high, low, change, changePercent, turnOver)
...
const crawlDetailHnxInvesting = asyncHandler(async (id, name, hrefDetail) => {
    ...
    try {
        ...
        let hnxInvestingDetailData = await page.evaluate(async (id, name) => {
            const delay = (m) => new Promise((r) => setTimeout(r, m));
