...
const crawlChartData1d = asyncHandler(async (nameId, rank) => {
    cron.schedule('* * * * * *', async () => {
        axios.get(`https://api.coingecko.com/api/v3/coins/${nameId}/market_chart?vs_currency=usd&days=1`)
            .then(response => {

                Chart1d.findOneAndUpdate({ name: nameId }, {
...
const crawlChartData7d = asyncHandler(async (nameId, rank) => {
    cron.schedule('* * * * * *', async () => {
        axios.get(`https://api.coingecko.com/api/v3/coins/${nameId}/market_chart?vs_currency=usd&days=7`)
            .then(response => {

                Chart7d.findOneAndUpdate({ name: nameId }, {
                    data: response.data.prices,
                    rank: rank
                }, { upsert: true }).catch(err => console.log(err))
            })
...
module.exports = { crawlChartData1d, crawlChartData7d, crawlChartData14d, crawlChartData30d, crawlChartData90d, crawlChartData1y, crawlChartDataMax }
