...
const hnxStockList = asyncHandler(async (req, res, next) => {
    const stockList = await Hnx.find({}).select('-_id -createdAt -updatedAt -__v')
...
const hnxDetailChart = asyncHandler(async (req, res, next) => {
    const symbol = req.params.symbol
    const list = await HnxChart.find({ symbol: symbol }).select('-_id -createdAt -updatedAt -__v')
    // let arr = []
...
module.exports = { hnxStockList, hnxDetailStock, hnxDetailReportChart, hnxDetailChart, hnxCompanyDetail }